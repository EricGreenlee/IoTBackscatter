...
const int packet_len_bits = 10;
int packet_bin[] = {1, 0, 1, 1, 1, 0, 0, 1, 0, 1};
int packet_bip[packet_len_bits];
...
//    dac_frequency_set(clk_8m_div, 1000);    //1000 ~=132kHz when clk_8m_div is set correctly
   ...
   Serial.println(READ_PERI_REG(SENS_SAR_DAC_CTRL2_REG),HEX);
    ...
    for (int i = 0; i < packet_len_bits; i++){
        if (packet_bin[i] == 0){
            packet_bip[i] = -1;
        }
        else{
...
void loop(){
    ...
    for (int bit = 0; bit < packet_len_bits; bit++){
        for(int i = 0; i < gc_len; i++){
            if (packet_bip[bit]*goldcode[2][i] == 1){
    ...
    delayMicroseconds(usec_delay*gc_len);
    dac_output_enable(DAC_CHANNEL_1);
    // for(int i = 0; i < gc_len; i++){
    //     if (goldcode[i] == 1){
    //         dac_invert_set(DAC_CHANNEL_1,2);
