...
int data[] = {1, 0, 1, 1, 1, 0, 0, 1, 0, 1};
const int data_len = 10;
int data_bip[data_len];
...
//    dac_frequency_set(clk_8m_div, 1000);    //1000 ~=132kHz when clk_8m_div is set correctly
   dac_frequency_set(clk_8m_div, 921);    //1000 ~=132kHz when clk_8m_div is set correctly
    ...
    Serial.println("Printing all DAC Sinusoidal registers:");
    Serial.println(READ_PERI_REG(SENS_SAR_DAC_CTRL1_REG),HEX);
    Serial.println(READ_PERI_REG(SENS_SAR_DAC_CTRL2_REG),HEX);
...
void loop(){
    ...
    // //uncomment this section for full demod
    // for (int bit = 0; bit < data_len; bit++){
    //     for(int i = 0; i < gc_len; i++){
    //         if (data_bip[bit]*goldcode[i] == 1){
    ...
    //     }
    // }
    // delayMicroseconds(usec_delay*10);
    
    //uncomment this section for carrier only
    dac_invert_set(DAC_CHANNEL_1,2);
    delayMicroseconds(usec_delay);
    
    // //uncomment this section for carrier with alternating phase only
    // dac_invert_set(DAC_CHANNEL_1,2);
    // delayMicroseconds(usec_delay);
    // dac_invert_set(DAC_CHANNEL_1,3);
    // delayMicroseconds(usec_delay);
