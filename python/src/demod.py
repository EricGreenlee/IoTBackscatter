...
def time_freq_code_search(samples, target_gc, input_sps, fs_hz):#, max_freq_dev_hz, nfreq_pts):
    # freq_offsets = 0
    # timedelay_sec = np.linspace(0, (len(samples)-1/fs_hz), len(samples))
    
    target_gc_stretch = np.repeat(target_gc,input_sps)
    correlation = np.correlate(samples, target_gc_stretch, mode='valid')
    
    # plt.figure()
    # plt.plot(correlation)
    # plt.grid("on")
    # plt.title("Correlation with Repeated Goldcode")
    
    threshold = 400 #sps*gclen*0.5
    ...
    if len(peaks) > 0:
        ...
        logger.info("first peak-> sample number: %s, value: %s, time: %s", first_peak_index, first_peak_value, first_peak_time_sec)
    else:
        logger.warning("No peaks found!")
        first_peak_index = 0
    out_samples = np.roll(samples, -1*first_peak_index)
...
def demodulate_packet(input_samples, tag_params, radio_params):
    ...
    for itag in range(tag_params.ntags):
        ...
        proc_samples = time_freq_code_search(proc_samples, itagParams.goldcode, itagParams.sps, radio_params.samplerate_hz)
